...
export async function loadDashboardData() {
  ...
  return {
    ...
    metadata: {
      ...
      repositorySource: repositoryPayload.source || "Nao informada",
      contributorSource: contributorPayload.source || "Nao informada",
      snapshotSources: Array.from(new Set((snapshotPayload.snapshots ?? []).map(item => item.source).filter(Boolean))),
      snapshotCount: snapshotPayload.snapshotCount ?? snapshotPayload.snapshots?.length ?? 0,
